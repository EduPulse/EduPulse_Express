...
router.post('/post', function (req, res, next) {
    ...
    try {
        Post.find({$and: [{$or: [{"article.versions.title": searchKey}, {"article.versions.content": searchKey}]}, {visibility: {$ne: "hidden"}}, {"article.status": "published"}]}).populate('post author', '').exec(function (err, results) {
            if (err) {
                console.error(err);
                res.sendStatus(500);
            }
